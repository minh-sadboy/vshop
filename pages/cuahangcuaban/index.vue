              ...
              <template v-slot:default>
                ...
                <tbody>
                  <tr v-for="data in datadh" :key="data.datadh">
                    ...
                    <td>
                      {{ Math.round(data.tonggiadh).toLocaleString("it-IT") }}₫
                    </td>
                    <td style="white-space: nowrap">
                      ...
                      <div v-if="data.trangthaidh == 5">
                        Người mua không nhận hàng (Hủy)
                      </div>
                      <div v-if="data.trangthaidh == 4">
                        Giao hàng thành công
                      </div>
                    </td>
                    <td v-if="data.trangthaidh == 1">
                      <v-btn
                        color="blue"
                        dark
                        small
                        :to="
                          '/cuahangcuaban/chitietdonhang?madonhang=' +
                          data.madonhang
                        "
                      >
                        Xác nhận
...
export default {
  ...
  head() {
    return {
      title: "Cửa hàng của tôi | vivishop",
    };
  },
  data() {
    return {
    ...
    if (this.$store.state.trangthaikh == 1) {
      this.$axios
        ...
        .then((response) => {
          ...
          this.$axios
            .post("http://localhost:3000/api/cuahangdh", {
