            ...
            <v-simple-table>
              <template v-slot:default>
                ...
                <tbody>
                  <tr>
                    <td>
                      {{ Math.round(doanhthushop).toLocaleString("it-IT") }}₫
                    </td>
                    <td>
                      {{ Math.round(thanhtoanshop).toLocaleString("it-IT") }}₫
                    </td>
                  </tr>
                </tbody>
              </template>
...
export default {
  ...
  head() {
    return {
      title: "Báo cáo cửa hàng | vivishop",
    };
  },
  data() {
    return {
  ...
  mounted() {
    if (this.$store.state.trangthaikh == 1) {
      this.$axios
        .post("http://localhost:3000/api/cuahangbaocao")
        .then((response) => {
          this.tenshop = response.data[0].tenshop;
          this.idshop = response.data[0].idshop;
          this.doanhthushop = response.data[0].doanhthushop;
          this.thanhtoanshop = response.data[0].thanhtoanshop;
        });
    } else {
