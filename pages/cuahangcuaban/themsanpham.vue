<template>
  <v-app>
    ...
    <v-app-bar fixed app dense>
      ...
      <v-app-bar-title> Thêm sản phẩm </v-app-bar-title>
      <NuxtLink to="/">
      ...
      <v-row no-gutters>
        ...
        <v-col cols="12" md="9">
          <div class="mx-5 my-4" :class="[$style.thongbaocaidat]">
            Quản lí sản phẩm
            <v-btn color="blue" outlined to="/cuahangcuaban/themsanphammoi">
              <v-icon>mdi-plus-box-outline</v-icon>&nbsp;Thêm
            </v-btn>
          </div>
          <v-card class="mx-5" outlined>
            <v-simple-table>
              <template v-slot:default>
                ...
                <tbody>
                  <tr v-for="data in datasp" :key="data.datasp">
                    <td>
                    ...
                    <td>
                      <div
                        ...
                        v-html="data.thongtin"
                      ></div>
                    </td>
                    <td>{{ data.gia }}</td>
                    <td>
                      <div style="min-width: 20px">-{{ data.giamgia }}%</div>
                    </td>
                    <td>
                      <div style="min-width: 20px">{{ data.sao }}⭐</div>
                    </td>
                    <td>
                      <div style="min-width: 20px">{{ data.soluongdg }}</div>
                    </td>
                    <td>
                      <div style="min-width: 65px">{{ data.cannangsp }}kg</div>
                    </td>
                    <td>
                      <div style="min-width: 65px">
                        <span v-if="data.trangthaisp == 1">Còn hàng</span>
                        <span v-if="data.trangthaisp == 0">Hết hàng</span>
                      </div>
                    </td>
                    <td>
                      <v-btn
                        ...
                        small
                        :to="
                          '/cuahangcuaban/suasanpham?dgltzq=' +
                          data.idsp +
                          '&awrzagw=' +
                          data.idshop
                        "
...
export default {
  ...
  head() {
    return {
      title: "Sản phẩm của hàng | vivishop",
    };
  },
  data() {
    return {
    ...
    if (this.$store.state.trangthaikh == 1) {
      this.$axios
        ...
        .then((response) => {
          ...
          this.$axios
            .post("http://localhost:3000/api/cuahangsp", {
              idshop: this.idshop,
            })
            .then((response1) => {
              this.datasp = response1.data;
            });
        });
    } else {
